        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    ...
                    <a href="{{ route('course.result') }}" class="btn btn-primary btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('update.Result', $result->id) }}" method="POST">
                    @csrf
                    @method('put')
                    <div class="form-group">
                        ...
                        <select class="form-control" name="questions[]" multiple id="question">
                            @foreach($questions as  $question)
                                <option {{ (in_array($question->id, old('questions', [])) || isset($result) && $result->questions->contains($question->id)) ? 'selected' : '' }} value="{{ $question->id }}">{{ $question->question_text }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
